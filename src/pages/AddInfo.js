...
const AddInfo = () => {
  ...
  //console.log("타겟 리스트", info_list && info_list.target);
  // const name = info_list.target;
  // console.log("마이네임", name);
  ...
  const [obstacleYN, setObstacleYN] = useState([]);
    //const [target, setTarget] = useState(info_list ? info_list.target : []);

  console.log("장애유형", obstacle);
  console.log("가구유형", target);
  console.log("성별", gender[0]);
  ...
  const [open_select_city, setOpenSelectCity] = React.useState(false);
  const [open_select, setOpenSelect] = React.useState(false);
  ...
  return (
    <MainWrap>
      ...
      <Container>
        <Grid>
          <Text bold margin="20px 8px">
            생년월일
          </Text>
          <CategoryBox>
            <input placeholder="년" onChange={infoYear} maxLength="4"></input>
            <input
              onChange={infoMonth}
              placeholder="월"
              maxLength="2"
              className="middle"
            ></input>
            <input placeholder="일" onChange={infoDate} maxLength="2"></input>

            {!year ? null : !birthYear(year) ? (
              <Grid is_flex>
                <ValidationBox style={{ color: "#ED6451" }}>
                  생년은 19YY, 20YY 형식으로 작성해주세요.
                </ValidationBox>
              </Grid>
            ) : null}
          ...
          <Text bold margin="40px 0 8px 8px">
            성별
          </Text>
          <CategoryBox>
      ...
      !birthDate(date) ? (
        ...
        <CompleteBtn
          onClick={() => {
            dispatch(
              infoActions.addInfoDB(userId, lifeCycle, gender, region, obstacleYN, obstacle)
            );
...
const CompleteBtn = styled.button`
  ...
  border-radius: 5px;
  border: none;
  background-color: ${(props) => (props.disabled === true ? "tomato" : "#E8E8E8")};
  color: ${(props) => (props.disabled === true ? "white" : "#999999")};
  font-weight: 900;
